...
def frequency_table(array:Union[np.ndarray, pd.Series], round:int=2, percent:bool=False)->pd.DataFrame:
    ...
    """
    size = len(array)    
    if size == 0: raise ValueError("빈 배열은 처리할 수 없습니다.")     # 경고문
        
    # 기본적인 빈도표 생성
    class_array, freq_array = np.unique(array, return_counts=True)
    freq_df = pd.DataFrame({'class':class_array, 'frequency':freq_array, 'ratio':freq_array/size})
...
def cross_table(
    ...
    # 비율 생성
    if ratio_style != 1:
        ratio_mat = _make_ratio_mat_for_cross_table(cross_df, vertical, percent, margin_txt)            # 비율(퍼센트) 행렬 계산
        ratio_mat = _ratio_style_for_cross_table(ratio_mat, percent, round, ratio_style)    # 반올림 및 교차표에 맞게 형태 정의

        # 비율을 교차표에 붙인다.
        if ratio_style == 0:
            cross_df = _convert_mat_to_cross_df_style_for_cross_table(cross_df, mat=np.char.add(cross_df.values.astype('<U9'), ratio_mat))
        # 비율을 교차표로 내보낸다.
        if ratio_style == 2:
...
# ==================================================
def _add_total_record_for_frequency_table(freq_df:pd.DataFrame, size:int)->pd.DataFrame:
    """
    ...
    """
    total_record = pd.DataFrame([('total', size, 1.0)], columns=['class', 'frequency', 'ratio'])
    return pd.concat([freq_df, total_record]).reset_index(drop=True)


def _ratio_to_percent_in_frequency_table(freq_df:pd.DataFrame, round:int)->None:
    """
    ...
    freq_df['%'] = freq_df['%'].round(round)
# ==================================================


# cross_table() 하위 함수
# ==================================================
def _margin_check_for_cross_table(df:pd.DataFrame, col1:str, col2:str, margin_txt:str):
    """
    ...
    if (margin_txt in col1_class) | (margin_txt in col2_class):
        raise ValueError(f"{margin_txt}가 이미 {col1} 또는 {col2}에 포함되어 있습니다. margin_txt 값을 변경하십시오.")
    

def _make_ratio_mat_for_cross_table(cross_df:pd.DataFrame, vertical:bool, percent:bool, margin_txt:str) -> np.ndarray:
    """
    교차표의 비율 행렬 생성
    ...
    if percent: ratio_mat = ratio_mat * 100     # 퍼센트 변환 여부
    return ratio_mat


def _ratio_style_for_cross_table(ratio_mat:np.ndarray, percent:bool, round:int, ratio_style:int) -> np.ndarray:
    """
...
def _convert_mat_to_cross_df_style_for_cross_table(cross_df:pd.DataFrame, mat:np.ndarray)->pd.DataFrame:
    """
